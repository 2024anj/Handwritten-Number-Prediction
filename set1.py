...
data = pd.read_csv('mnist_test_0_1.csv')
...
def matrices():
    #rand generates array full of random values between 0-1
    #subtract by .5 to get values between -.5-.5
    W1 = np.random.rand(2, 784) - .5   #2x784
    W2 = np.random.rand(2, 2) - .5     #2x2
    b1 = np.random.rand(2, 1) - .5     #2x1
    b2 = np.random.rand(2, 1) - .5    #2x1
    return W1, b1, W2, b2
...
def backProp(A1, A2, W1, b1, b2, W2, Z1, X, Y, alpha):
    ...
    oneHot = np.zeros((EXAMPLES, 2)) #creates new matrix filled w 0s, size:12664x2
    ...
    W1 -= alpha * der_W1
    b1 -= alpha * np.reshape(der_b1, (2,1))#deriv bias 1 to 2x1 matrix
    W2 -= alpha * der_W2
    b2 -= alpha * np.reshape(der_b2, (2,1))#deriv bias 2 to 2x1 matrix
    return W1, b1, W2, b2
...
print('--------------------------------------------')
testing(W1, b1, W2, b2, 1) #to test other, change last parameter
